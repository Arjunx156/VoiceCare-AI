...
# Order codes are read aloud, so they come back from speech recognition with
# stray spaces, hyphens and dots. Compare on letters and digits only.
_ORDER_CODE_NOISE_RE = re.compile(r"[^A-Z0-9]")
_ORDER_CODE_PREFIX = "ORD"
# Length of the code body generated by utils/short_ids.generate_order_number.
_MIN_ORDER_CODE_CHARS = 4
...
class VoiceCarePipeline:
    ...
    async def agent_order_lookup(self, state: PipelineState) -> PipelineState:
        ...
        try:
            ...
            order_ref = state.input_order_id or state.extracted_order_id
            ...
            # one and nothing else is still a lookup we can complete — this
            # branch is why agent 3 no longer returns in 0 ms when the only
            # thing the customer said was their order number.
            order = None
            if order_ref:
                # Scoped to the account when a phone already matched one, so a
                # guessed phone can never have someone else's order attached.
                order = await self._resolve_order(
                    order_ref, user.user_id if user else None
                )
                state.order_not_found = order is None
                if order is not None and user is None:
                    user = (await self.db.execute(
                        select(User).where(User.user_id == order.user_id).options(noload("*"))
                    )).scalar_one_or_none()
            ...
            if user:
                ...
                # No order named: fall back to the account's latest one, which
                # is what "where is my order" means for most callers.
                if order is None and not order_ref:
                ...
                if not (state.identity_verified or order_corroborates or name_corroborates):
                    ...
                    await self._load_order_context(state, order)
            ...
            if state.lookup_successful:
                decision = "Data retrieved from Postgres"
            elif state.identity_needs_confirmation:
                decision = "Name match — awaiting identity confirmation"
            elif state.order_not_found:
                decision = f"No order matching '{order_ref}'"
            elif not (phone or order_ref or state.extracted_name):
                decision = "No identifier supplied — nothing to look up"
            else:
                decision = "No matching records"

            state.add_trace(
                agent_name="Order Lookup",
                stage_number=3,
                input_summary=f"Phone: {phone}, Order: {order_ref}",
            ...
            logger.error("order_lookup_failed", error=str(e), exc_info=True)
            state.lookup_successful = False

        return state

    async def _resolve_order(
        self, identifier: str, user_id: Optional[uuid.UUID] = None
    ) -> Optional[Order]:
        """Find an order from whatever the customer actually said.

        Customers speak the short code — "ORD-7K3F" — because that is the only
        form they are ever shown or read back. This used to run `uuid.UUID()`
        on it, raise ValueError, log `invalid_order_uuid_format` and return no
        order, so a correct order number looked exactly like a wrong one.

        Both forms are accepted now, and the short code is matched on its
        letters and digits only, because it arrives through speech recognition:
        "ord 7k3f", "ORD 7K 3F" and "ORD-7K3F" are the same order.
        """
        if not identifier:
            return None
        ...
        clauses = []
        try:
            clauses.append(Order.order_id == uuid.UUID(str(identifier)))
        ...
        if len(code) >= _MIN_ORDER_CODE_CHARS and code != _ORDER_CODE_PREFIX:
            variants = {code}
            if not code.startswith(_ORDER_CODE_PREFIX):
                # "my order is 7K3F" — the customer dropped the prefix.
                variants.add(f"{_ORDER_CODE_PREFIX}{code}")
            stored_code = func.replace(
                func.replace(func.upper(Order.order_number), "-", ""), " ", ""
            )
            clauses.append(stored_code.in_(sorted(variants)))

        if not clauses:
            logger.info("order_identifier_unusable", identifier=str(identifier)[:32])
            return None

        stmt = select(Order).where(or_(*clauses)).options(noload("*")).limit(1)
        if user_id is not None:
            stmt = stmt.where(Order.user_id == user_id)
        return (await self.db.execute(stmt)).scalars().first()

    async def _load_order_context(self, state: PipelineState, order: Order) -> None:
        """Load shipment, return, refund and payment rows for a verified order.

        Sequential rather than gathered on purpose: one AsyncSession cannot
        serve concurrent statements, and agent 3 already runs alongside agent 4.
        """
        shipment = (await self.db.execute(
    ...
    async def agent_resolution(self, state: PipelineState) -> PipelineState:
        ...
        try:
            ...
            query = state.transcript_english or state.transcript_original
            # Everything agent 3 loaded, not just the order row: a refund
            # question cannot be answered well from the order status alone.
            result = await self.gemini.generate_resolution(
                ...
                order_not_found=state.order_not_found,
                order_reference=state.input_order_id or state.extracted_order_id,
            )
